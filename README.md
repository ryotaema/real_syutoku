...
└── data/
    ...
    ├── mp4/                 # 録画ファイル
    ├── pointcloud/          # 点群データ（YYYY-MM-DD/session_HHMMSS/）
...
| ir_left / ir_right | ✓ | なし |
| pointcloud | ✓ | ✓ |
| 深度表示 alpha | 0.4（~637mm 飽和） | 0.5（~510mm 飽和） |

D405 接続時は IR ストリームを無効化し，color と depth のみを収集します．深度カラーマップの alpha 値はカメラの距離範囲に合わせて自動切り替えされます．
...
camera:
  ...
  depth_alpha:
    D435: 0.4   # ~637mm で飽和
    D405: 0.5   # ~510mm で飽和（D405 最大距離 ~500mm に合わせる）
    default: 0.4
...
```bash
# 接続カメラを自動検出して適切なスクリプトを起動
python3 collect/auto_collect.py
python3 collect/auto_collect.py --fps 15   # CLI オプションも転送される
```

| 検出カメラ | 起動スクリプト | 挙動 |
|---|---|---|
| D435 | `dataset_collect.py` | Enter キーで収集開始 |
| D405 | `d405_dataset_collect.py` | 連続 30FPS で自動保存 |
| 未対応機種 | `dataset_collect.py`（フォールバック） | 警告を表示して起動 |
...
# 連続 30FPS で自動保存，q で停止（D435/D405 両対応）
